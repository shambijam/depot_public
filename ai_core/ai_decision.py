...
class AIDecision:
    ...
    def audit_trading_performance(
        ...
        Audite la performance de trading du bot en analysant les logs historiques
        pour une période donnée, en utilisant un prompt externalisé.
        Génère des suggestions d'amélioration de la performance, avec un bilan par actif,
        les raisons des gains/pertes, et l'analyse par phase de marché.
        Ce rapport est sauvegardé dans le dossier /config/ai_audit.
        L'IA ne calcule AUCUN score de confiance ou de priorité.
        ...
        Args:
            logs (List[Dict[str, Any]]): Liste des entrées de logs de trading pertinentes pour l'audit.
                                        Ces logs doivent idéalement être les entrées complètes de `ConfigManager.log_decision`.
            period (str): La période de l'audit (ex: "last_day", "last_week", "last_month").
            current_context (Optional[Dict[str, Any]]): Le contexte actuel de l'environnement, passé explicitement pour l'IA.
        ...
        for log_entry in logs:
            if (
                ...
                if outcome.get("status") == "executed" and "pnl_usd" in outcome:
                    ...
                    phase_at_trade = market_data_summary.get("phase", "N/A")
                    # Suppression de 'confidence_at_trade' car l'IA ne gère plus la confiance
                    ...
                    trade_details_for_ai.append(
                        {
                            ...
                            "entry_signals": market_data_summary,
                        }
                    )

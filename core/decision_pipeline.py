...
class DecisionPipeline:
    ...
    def select_assets_to_trade(self, context: Dict[str, Any]) -> List[str]:
        ...
        if not opportunities_candidates:
            self.logger.info("Aucune opportunité candidate à filtrer pour l'IA (liste vide).")
            return []

        final_opportunities_for_ai = []

        major_fx_pairs = self.config_manager.get("ai.opportunity_filtering.major_fx_pairs_for_correlation", ["EURUSD", "GBPUSD", "USDJPY"],)
        ...
        for asset in opportunities_candidates:
            ...
            phases_of_interest_for_asset = asset_specific_config.get("phases_of_interest", {})
            phase_type = current_asset_phase.split("_")[0]

            if (phases_of_interest_for_asset and phase_type not in phases_of_interest_for_asset):
                self.logger.debug(f"Actif {asset} écarté : phase '{current_asset_phase}' non explicitement listée comme d'intérêt dans la config de l'actif.")
        ...
        self.logger.info(f"Shortlist d'opportunités pour l'IA après filtrage : {final_opportunities_for_ai}")
        return final_opportunities_for_ai


    def score_configs(self, context: Dict[str, Any], configs: Dict[str, Any]) -> Dict[str, float]:
        """
        ...
        weekend_crypto_priority_enabled = self.config_manager.get("scoring_rules.weekend_crypto_priority.enabled", False)
        if is_weekend and weekend_crypto_priority_enabled:
            self.logger.info("Mode week-end : Priorité aux stratégies crypto.")
            crypto_tag = self.config_manager.get("scoring_rules.weekend_crypto_priority.strategy_tag", "crypto")

            return {
                path: (1.0 if crypto_tag in data["content"].get("strategy_tags", []) else 0.0)
        ...
        for path, data in configs.items():
            ...
            self.logger.debug(f"Scoring stratégie '{strategy_name}' (Tags: {strategy_tags})")

            for tag, weight in strategy_weights.items():
                if tag in strategy_tags and tag in market_regime:
                    score += weight
                    self.logger.debug(f"  + Score pour tag '{tag}' correspondant au régime. Score: {score}")
                elif (
                    ...
                    score += weight
                    self.logger.debug(f"  + Score pour compatibilité ancienne de tag/régime. Score: {score}")

            risk_appetite = context.get("risk_appetite", "medium")
            max_dd = config.get("max_drawdown_percent", 5.0)

            if risk_appetite == "low" and max_dd < risk_thresholds.get("low_risk_max_drawdown", 3.0):
                score += risk_thresholds.get("low_risk_score_boost", 0.1)
                self.logger.debug(f"  + Score boost pour appétit au risque 'bas' et faible DD. Score: {score}")
            elif risk_appetite == "high" and max_dd > risk_thresholds.get("high_risk_min_drawdown", 7.0):
                score += risk_thresholds.get("high_risk_score_boost", 0.05)
                self.logger.debug(f"  + Score boost pour appétit au risque 'élevé' et DD plus important. Score: {score}")

            # L'IA fournit maintenant un score de *pertinence stratégique* (ai_vote_for_configs)
            # que le DecisionPipeline peut intégrer dans son scoring.
            ai_recommendation_for_this_config_score = context.get('ai_recommendation_score', {}).get(Path(path).name, 0.0) # Utilise le nom du fichier comme clé
            ai_weight = self.config_manager.get("scoring_rules.ai_recommendation_weight", 0.2) # Nouveau paramètre
            score += ai_recommendation_for_this_config_score * ai_weight
            self.logger.debug(f"  + Score AI de pertinence pour '{strategy_name}': {ai_recommendation_for_this_config_score * ai_weight}. Score: {score}")


            historical_performance = data.get("performance", {})
            if historical_performance:
                sharpe_ratio = historical_performance.get("sharpe_ratio", 0.0)
                if sharpe_ratio > self.config_manager.get("scoring_rules.performance_thresholds.good_sharpe", 1.0):
                    score += self.config_manager.get("scoring_rules.performance_thresholds.good_sharpe_boost", 0.1)
                elif sharpe_ratio < self.config_manager.get("scoring_rules.performance_thresholds.poor_sharpe", 0.5):
                    score -= self.config_manager.get("scoring_rules.performance_thresholds.poor_sharpe_penalty", 0.1)
                self.logger.debug(f"  + Score performance historique (Sharpe: {sharpe_ratio}). Score: {score}")

            config_scores[path] = max(0.0, min(1.0, score))

        self.logger.info(f"Évaluation des configurations terminée. Scores : {config_scores}")
        return config_scores

    def select_optimal_config(self, context: Dict[str, Any], configs: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if scored_configs:
            sorted_configs = sorted(scored_configs.items(), key=lambda item: item[1], reverse=True)
            optimal_config_path, optimal_score = sorted_configs[0]

            min_optimal_score_threshold = self.config_manager.get("scoring_rules.min_optimal_score_threshold", 0.1)
            if optimal_score < min_optimal_score_threshold:
                self.logger.warning(f"Le score optimal ({optimal_score:.2f}) est inférieur au seuil minimal ({min_optimal_score_threshold:.2f}). Cherche une stratégie par défaut.")
                optimal_config_path = None

        if optimal_config_path is None:
            self.logger.warning("Aucune configuration optimale trouvée par le scoring ou le score est trop bas. Tentative de chargement de la stratégie par défaut.")

            default_strategy_key = self.config_manager.get("strategies.default_strategy")
            if default_strategy_key:
                ...
                if default_strategy_file_name:
                    actual_config_dir = Path(self.config_manager.get("paths.strategy_configs", "config/"))
                    default_strategy_file_path = actual_config_dir / default_strategy_file_name
                    ...
                    self.logger.error(f"Aucun fichier de configuration mappé pour la stratégie par défaut '{default_strategy_key}'. Impossible de fournir un fallback.")
                    return {}
            else:
    ...
    def adapt_config(self, config: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        self.logger.debug(f"Adaptation pour stratégie '{strategy_name}' (Tags: {strategy_tags}) - Régime: {market_regime}, Volatilité: {volatility_percentage:.2f}%")

        adapted_config = self._adapt_to_volatility(adapted_config, volatility_percentage)
        adapted_config = self._adapt_to_market_regime(adapted_config, market_regime, strategy_tags)

        self.logger.info("Adaptation de la configuration terminée.")
        return adapted_config

    def _adapt_to_volatility(self, config: Dict[str, Any], volatility_percentage: float) -> Dict[str, Any]:
        """
        Adapte les paramètres de la configuration en fonction de la volatilité actuelle.
        Déplacée de ConfigManager.
        """
        adapted_config = config.copy()
        high_vol_threshold = self.config_manager.get("adaptation_settings.volatility_thresholds.high", 25.0)
        low_vol_threshold = self.config_manager.get("adaptation_settings.volatility_thresholds.low", 15.0)

        if volatility_percentage > high_vol_threshold:
            self.logger.debug(f"Contexte de HAUTE VOLATILITÉ détecté ({volatility_percentage:.2f}% > {high_vol_threshold}%). Application des ajustements.")

            if "scalping" in adapted_config.get("strategy_tags", []):
                adapted_config["take_profit_pips"] = self.config_manager.get("adaptation_settings.scalping.take_profit_pips_high_vol", config.get("take_profit_pips"),)
                adapted_config["stop_loss_pips"] = self.config_manager.get("adaptation_settings.scalping.stop_loss_pips_high_vol", config.get("stop_loss_pips"),)
                adapted_config["max_spread_pips"] = self.config_manager.get("adaptation_settings.scalping.max_spread_pips_high_vol", config.get("max_spread_pips"),)
                self.logger.debug(f"  Adaptation Scalping (Haute Vol) : SL/TP ajustés à {adapted_config.get('stop_loss_pips')}/{adapted_config.get('take_profit_pips')} pips, Spread max: {adapted_config.get('max_spread_pips')} pips.")

            risk_reduction_multiplier = self.config_manager.get("adaptation_settings.risk_adjustment.risk_reduction_multiplier_high_vol", 0.8,)
            min_risk = self.config_manager.get("adaptation_settings.risk_adjustment.min_risk_percent_after_adjustment", 0.01,)
            original_risk = adapted_config.get("risk_per_trade_percent", 1.0)
            adapted_config["risk_per_trade_percent"] = max(min_risk, original_risk * risk_reduction_multiplier)
            ...
            self.logger.debug(f"Contexte de BASSE VOLATILITÉ détecté ({volatility_percentage:.2f}% < {low_vol_threshold}%). Application des ajustements.")

            if "scalping" in adapted_config.get("strategy_tags", []):
                adapted_config["take_profit_pips"] = self.config_manager.get("adaptation_settings.scalping.take_profit_pips_low_vol", config.get("take_profit_pips"),)
                adapted_config["stop_loss_pips"] = self.config_manager.get("adaptation_settings.scalping.stop_loss_pips_low_vol", config.get("stop_loss_pips"),)
    ...
    def _adapt_to_market_regime(self, config: Dict[str, Any], market_regime: str, strategy_tags: List[str]) -> Dict[str, Any]:
        """
        ...
        if "consolidation" in market_regime and "range" in strategy_tags:
            ...
            self.logger.debug(f"Contexte de TENDANCE détecté. Adaptation pour stratégie de suivi de tendance.")
            adapted_config["take_profit_pips"] = self.config_manager.get("adaptation_settings.market_regime_specific.trending.take_profit_pips", config.get("take_profit_pips"),)
            adapted_config["stop_loss_pips"] = self.config_manager.get("adaptation_settings.market_regime_specific.trending.stop_loss_pips", config.get("stop_loss_pips"),)
            adapted_config["enable_trailing_stop"] = self.config_manager.get("adaptation_settings.market_regime_specific.trending.enable_trailing_stop", adapted_config.get("enable_trailing_stop"),)
            adapted_config["risk_per_trade_percent"] = self.config_manager.get("adaptation_settings.market_regime_specific.trending.risk_per_trade_percent", config.get("risk_per_trade_percent"),)

        return adapted_config

    def decide_exit_trades(self, context: Dict[str, Any], open_positions: List[Dict[str, Any]], active_config: Dict[str, Any], strategy_manager_instance) -> List[Dict[str, Any]]:
        """
        ...
        """
        self.logger.info("Orchestration de la décision de sortie en déléguant aux stratégies actives...")
        all_exit_decisions = []
        ...
        if strategy_manager_instance:
            magic_to_strategy_map = strategy_manager_instance.get_magic_to_strategy_map() # Nouvelle méthode à créer dans StrategyManager
        else: # Fallback si StrategyManager n'est pas encore injecté ou fonctionnel
            self.logger.warning("StrategyManager non injecté. La logique de décision de sortie pourrait être limitée.")
            ...
            if hasattr(self.config_manager, '_config_knowledge_base'):
                 for config_data in self.config_manager._config_knowledge_base.values():
                    ...
                    if magic and strategy_class:
                        magic_to_strategy_map[magic] = {"class": strategy_class, "config": content}
        ...
        for magic, positions in positions_by_strategy.items():
            if magic in magic_to_strategy_map:
                ...
                self.logger.info(f"Évaluation des sorties pour {len(positions)} position(s) de la stratégie '{strategy_name}' (Magic: {magic}).")
                try:
                    # Passe l'instance de ConfigManager à la stratégie
                    strategy_instance = strategy_class(config_manager_instance=self.config_manager, strategy_config=strategy_config)
                    exit_decisions_for_strategy = strategy_instance.evaluate_exit(context, positions)

                    if exit_decisions_for_strategy:
                        all_exit_decisions.extend(exit_decisions_for_strategy)
                        self.logger.info(f"{len(exit_decisions_for_strategy)} décision(s) de sortie retournée(s) par la stratégie '{strategy_name}'.")

                except Exception as e:
                    self.logger.error(f"Erreur lors de l'évaluation des sorties pour la stratégie '{strategy_name}': {e}", exc_info=True)
            else:
                self.logger.warning(f"Aucune stratégie trouvée pour le magic number {magic}. Les {len(positions)} position(s) associées ne peuvent pas être gérées pour la sortie.")

        return all_exit_decisions

    def decide_trade_to_execute(self, context: Dict[str, Any], current_config: Dict[str, Any], signals: Dict[str, Any], strategy_manager_instance=None) -> Dict[str, Any]:
        """
        ...
        self.logger.info(f"Délégation de la décision d'entrée à l'instance de : {strategy_class.__name__}")
        trade_decision = None
        try:
            # Passe l'instance de ConfigManager à la stratégie
        ...
        if not trade_decision:
            self.logger.info(f"La stratégie '{strategy_name}' n'a trouvé aucune opportunité d'entrée ce cycle.")
            return {}
    ...
    def _compare_values(self, actual_value, operator, expected_value) -> bool:
        ...
        if not isinstance(actual_value, (int, float)):
            return False
        if operator == ">": return actual_value > expected_value
        if operator == "<": return actual_value < expected_value
        if operator == ">=": return actual_value >= expected_value
        if operator == "<=": return actual_value <= expected_value
        if operator == "==": return actual_value == expected_value
    ...
    def calculate_risk_parameters(self, context: Dict[str, Any], config: Dict[str, Any], trade_decision: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        equity = context.get("account_info", {}).get("equity", self.config_manager.get("risk_management_settings.default_account_equity", 10000.0),)
        if equity <= 0:
        ...
        point = asset_mt5_info.get("point", self.config_manager.get("risk_management_settings.default_points_in_pip", 0.00001))
        contract_size = asset_mt5_info.get("trade_contract_size", self.config_manager.get("risk_management_settings.default_contract_size", 100000))
        ...
        if dollar_risk_per_lot_estimated <= 0:
            ...
            dollar_risk_per_lot_estimated = self.config_manager.get("risk_management_settings.min_dollar_risk_per_lot_fallback", 1.0)

        calculated_lot_size = max_dollar_risk / dollar_risk_per_lot_estimated

        min_lot_size = account_trade_settings.get("min_lot", self.config_manager.get("risk_management_settings.min_lot_size_fallback", 0.01))
        max_lot_size = account_trade_settings.get("max_lot", self.config_manager.get("global_safety.max_allowed_lot_size", 50.0))
        lot_step = account_trade_settings.get("lot_step", self.config_manager.get("risk_management_settings.default_lot_step_fallback", 0.01))

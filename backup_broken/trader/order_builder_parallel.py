...
# 🚨 IMPORTANT: Ajouter l'import de l'exception
...
async def run_validations_parallel(
    ...
    try:
        ...
        if not raw_symbol or raw_symbol == "UNKNOWN":
            # 🚨 PROPAGER l'erreur plutôt que fallback
            raise TradeExecutionError(f"Symbole invalide: '{raw_symbol}'")
        ...
        # Validation des résultats MT5
        if not symbol_info:
            raise TradeExecutionError(f"Symbole MT5 introuvable: {raw_symbol}")
        ...
        # 🚨🚨🚨 SUPPRIMER LE FALLBACK TOXIQUE ! 🚨🚨🚨
        # NE PAS RETENTER avec validate_fn()
        # Propager l'erreur pour que le système la gère
        raise TradeExecutionError(f"Validation parallèle échouée: {e}") from e
...
def prepare_order_parallel_wrapper(
    ...
    try:
        ...
        # 🚨 PROPAGER l'erreur plutôt que de faire un fallback silencieux
        # Le système de trading principal doit décider si retenter
        raise TradeExecutionError(f"Échec préparation d'ordre (mode parallèle): {e}")
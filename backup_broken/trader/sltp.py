...
def calculate_pip_value_per_lot(
    ...
    """
    Calcule la valeur d'un pip pour 1 lot standard (en USD)
    Version sans dépendre de _pip_size_for_symbol
    """
    from typing import Any
    
    symbol = symbol.upper().strip()
    
    # 1. CAS SPÉCIFIQUES POUR VOS SYMBOLES
    # NAS100 / US100
    if "NAS" in symbol or "US100" in symbol:
        pip_value = 20.0
        if logger:
            logger.info(f"💰 [PIP_VALUE][{symbol}] Valeur fixe: {pip_value} USD/point")
        return pip_value
    
    # USDJPY
    if "USDJPY" in symbol:
        if current_rate and current_rate > 0:
            pip_value_usd = 1000.0 / float(current_rate)
            if logger:
                logger.info(f"💰 [PIP_VALUE][{symbol}] @{current_rate}: {pip_value_usd:.4f} USD")
            return pip_value_usd
        else:
            fallback_rate = 150.0
            pip_value_usd = 1000.0 / fallback_rate
            if logger:
                logger.info(f"💰 [PIP_VALUE][{symbol}] Taux inconnu, using {fallback_rate}: {pip_value_usd:.4f} USD")
            return pip_value_usd
    ...
    if "GBPUSD" in symbol:
        pip_value = 10.0
        if logger:
            logger.info(f"💰 [PIP_VALUE][{symbol}] Valeur fixe: {pip_value} USD")
        return pip_value
    
    # 2. UTILISER DIRECTEMENT SYMBOL_INFO SANS _pip_size_for_symbol
    if symbol_info is not None:
        try:
            # Récupérer trade_tick_value_laverage directement
            tick_value = getattr(symbol_info, "trade_tick_value", 0)
            tick_value = float(tick_value) if tick_value else 0
            
            # Récupérer trade_tick_size
            tick_size = getattr(symbol_info, "trade_tick_size", 0)
            tick_size = float(tick_size) if tick_size else 0
            
            # Récupérer digits pour déterminer la taille du pip
            digits = int(getattr(symbol_info, "digits", 5) or 5)
            
            if tick_value > 0 and tick_size > 0:
                # Déterminer pip_size_local basé sur digits
                if digits in (3, 5):
                    # Pour forex avec 3 ou 5 digits, 1 pip = 10 points
                    pip_multiplier = 10.0
                else:
                    # Pour autres, 1 pip = 1 point
                    pip_multiplier = 1.0
                
                # Calcul direct sans _pip_size_for_symbol
                pip_value_local = pip_multiplier * tick_value
                
                if logger:
                    logger.info(f"💰 [PIP_VALUE][{symbol}] Calcul direct MT5:")
                    logger.info(f"   - digits: {digits}")
                    logger.info(f"   - tick_size: {tick_size}")
                    logger.info(f"   - tick_value: {tick_value}")
                    logger.info(f"   - pip_multiplier: {pip_multiplier}")
                    logger.info(f"   - pip_value: {pip_value_local:.6f} USD")
                
                return pip_value_local
                
        except Exception as e:
            if logger:
                logger.warning(f"⚠️ [PIP_VALUE][{symbol}] Erreur symbol_info: {str(e)}")
    
    # 3. VALEURS FIXES POUR SYMBOLES COURANTS
    # Forex majors
    forex_majors = ["EURUSD", "GBPUSD", "AUDUSD", "NZDUSD", "USDCAD", "USDCHF"]
    if symbol in forex_majors:
        pip_value = 10.0
        if logger:
            logger.info(f"💰 [PIP_VALUE][{symbol}] Forex major: {pip_value} USD")
        return pip_value
    ...
    if "XAU" in symbol or "GOLD" in symbol:
        pip_value = 1.0
        if logger:
            logger.info(f"💰 [PIP_VALUE][{symbol}] Or: {pip_value} USD")
        return pip_value
    
    # Pétrole
    if "OIL" in symbol or "WTI" in symbol or "XTI" in symbol:
        pip_value = 10.0
        if logger:
            logger.info(f"💰 [PIP_VALUE][{symbol}] Pétrole: {pip_value} USD")
        return pip_value
    
    # Paires JPY (sauf USDJPY déjà traité)
    if "JPY" in symbol and "USDJPY" not in symbol:
        usdjpy_rate = current_rate if current_rate else 150.0
        pip_value = 1000.0 / usdjpy_rate
        if logger:
            logger.info(f"💰 [PIP_VALUE][{symbol}] JPY paire: ~{pip_value:.4f} USD")
        return pip_value
    
    # 4. HEURISTIQUE BASÉE SUR LE NOM DU SYMBOLE
    # Si se termine par USD (EURUSD, AUDUSD, etc.)
    if symbol.endswith("USD"):
        pip_value = 10.0
        if logger:
            logger.info(f"💰 [PIP_VALUE][{symbol}] Finit par USD: {pip_value} USD")
        return pip_value
    
    # Si commence par USD (USDCAD, USDCHF, etc.)
    if symbol.startswith("USD"):
        pip_value = 10.0
        if logger:
            logger.info(f"💰 [PIP_VALUE][{symbol}] Commence par USD: {pip_value} USD")
        return pip_value
    
    # 5. FALLBACK
    pip_value = 10.0
    if logger:
        logger.warning(f"⚠️ [PIP_VALUE][{symbol}] Fallback: {pip_value} USD")

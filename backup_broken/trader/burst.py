...
try:
    ...
    # Si l'import échoue, définir une fonction locale
    def calculate_pip_value_per_lot(
        symbol: str, symbol_info: Any = None, current_rate: float = None, logger=None
    ) -> float:
        ...
        if "NAS" in symbol or "US100" in symbol:
            ...
            return 1000.0 / (
                current_rate if current_rate and current_rate > 0 else 150.0
            )
        elif "GBPUSD" in symbol:
            return 10.0
        elif symbol.endswith("USD") or symbol.startswith("USD"):
            return 10.0
        else:
            return 10.0
...
def monitor_burst_baskets(
    ...
    target_profit_pips: float = None,
    trail_trigger: float = 10.0,
    trail_step: float = 5.0,
    config_manager_instance=None,  # <--- AJOUTEZ CET ARGUMENT ICI
    **_,
):
    """
    ...
    # ---- Conf / garde-fous ----
    # ⚠️ CORRECTION: closure_rules n'est plus dans entry_rules.scalping.burst_scalping
    # Il est maintenant dans overrides.scalping.closure_rules ou racine après fusion

    # Utiliser config_manager pour récupérer closure_rules
    cm = (
        config_manager_instance
        if config_manager_instance
        else getattr(self, "config_manager", None)
    )
    # Toujours définir burst_cfg (évite NameError plus bas si require_full_count_for_profit_close=True)
    burst_cfg = (
    ...
    if not cm:
        ...
        if logger:
            logger.error(
                "[BASKET_MONITOR] ConfigManager non disponible! Utilisation fallback..."
            )
        if logger:
            logger.info(f"[BASKET_MONITOR] running from: {__file__}")

        # Fallback: ancienne méthode (ne fonctionnera pas mais c'est mieux que rien)
        burst_cfg = (
            config.get("entry_rules", {}).get("scalping", {}).get("burst_scalping", {})
        ) or {}
        closure = burst_cfg.get("closure_rules", {}) or {}
    else:
        # ✅ NOUVELLE MÉTHODE: Récupérer closure_rules depuis config_manager
        # 🔧 FIX 23 JAN 2026: Fusionner les valeurs de config avec les défauts

        # D'abord récupérer closure_rules depuis la config passée
        config_closure = burst_cfg.get("closure_rules", {}) or {}

        # Valeurs par défaut (seront écrasées par config_closure si présentes)
        closure = {
            ...
            "rt_fast_window_ms": 120000,  # 🔧 FIX: 2 minutes par défaut (était 0)
            "rt_poll_interval_ms": 120,
            "loss_guard_arming_ms": 3000,
            "min_age_ms_for_any_close": 3000,
        }

        # 🔧 FIX CRITIQUE: Fusionner avec la config (écrase les défauts)
        closure.update(config_closure)

        max_loss_pips = float(max_loss_pips)
    ...
    # 🔧 FIX 23 JAN 2026: Utiliser config_manager_instance si disponible
    try:
        cm_for_magic = (
            config_manager_instance
            if config_manager_instance
        ...
        BOT_MAGIC = int(
            getattr(self, "magic", 0)
            or (cm_for_magic.get("magic_number", 0) if cm_for_magic else 0)
            or (cm_for_magic.get("defaults.magic_number", 0) if cm_for_magic else 0)
            or 0
        )
        if logger and BOT_MAGIC:
            logger.debug(f"[BASKET_MONITOR] BOT_MAGIC={BOT_MAGIC} (source: config_manager)")
    except Exception as e:
        BOT_MAGIC = 0
        if logger:
            logger.warning(f"[BASKET_MONITOR] BOT_MAGIC=0 (erreur: {e})")
    ...
    # ---------- Helpers ----------
    # ✅ FIX: accepter hex en maj/min + rendre "bs_" insensible à la casse
    BASKET_TAG_RE = re.compile(r"bs_([A-Fa-f0-9]{8})", re.IGNORECASE)
    ...
    def _pip_size_for_symbol(sym: str) -> float:
        """Calcule la taille d'un pip en termes de prix"""
        sym_upper = sym.upper()

        # Gestion spécifique pour NAS100/US100 (indices)
        if "NAS" in sym_upper or "US100" in sym_upper or "NDX" in sym_upper:
            # NAS100: 1 point = 1 unité (pas de concept traditionnel de pip)
            # Pour le calcul de risque, on traite 1 point comme 1 pip
            return 1.0

        # Obtenir les infos du symbole depuis MT5
        si = _symbol_info(sym)
        point = _safe_float(_gv(si, "point", 0.0001), 0.0001) or 0.0001
        digits = int(_gv(si, "digits", 5) or 5)

        # Déterminer les points par pip selon les digits
        # digits=3,5 => 1 pip = 10 points, sinon 1 pip = 1 point
        if digits in (3, 5):
            points_per_pip = 10.0
        elif digits == 2:
            # Pour les indices comme NAS100 (géré plus haut) et métaux
            points_per_pip = 1.0
        else:
            points_per_pip = 1.0
        ...
        # DEBUG: Log UNE SEULE FOIS par symbole
        if logger and not hasattr(_pip_size_for_symbol, "_debug_done"):
            _pip_size_for_symbol._debug_done = {}

        if logger and sym not in getattr(_pip_size_for_symbol, "_debug_done", {}):
            _pip_size_for_symbol._debug_done[sym] = True

            # Informations spécifiques pour vos symboles
            if "USDJPY" in sym_upper:
                expected_pip = 0.01
                pip_type = "USDJPY (1 pip = 0.01)"
            elif "GBPUSD" in sym_upper:
                expected_pip = 0.0001
                pip_type = "GBPUSD (1 pip = 0.0001)"
            elif "NAS" in sym_upper or "US100" in sym_upper:
                expected_pip = 1.0
                pip_type = "NAS100 (1 point = 1.0)"
            elif "XAU" in sym_upper or "GOLD" in sym_upper:
                expected_pip = 0.1
                pip_type = "Or (1 pip = 0.1)"
            elif digits == 5:
                expected_pip = 0.0001
                pip_type = "Forex 5 digits (1 pip = 0.0001)"
            elif digits == 3:
                expected_pip = 0.001
                pip_type = "3 digits (1 pip = 0.001)"
            elif digits == 2:
                expected_pip = 0.01
                pip_type = "2 digits (1 pip = 0.01)"
            else:
                expected_pip = pip_size
                pip_type = f"Autre (digits={digits})"

            logger.info(f"🔢 [PIP_CALC_INIT][{sym}]")
            logger.info(f"   - Type: {pip_type}")
            logger.info(f"   - Point: {point}")
            logger.info(f"   - Digits: {digits}")
            logger.info(f"   - Points per pip: {points_per_pip}")
            logger.info(f"   - Calculated pip size: {pip_size}")
            logger.info(f"   - Expected 1 pip: {expected_pip}")

            # Vérification pour USDJPY
            if "USDJPY" in sym_upper:
                if abs(pip_size - 0.01) > 0.000001:
                    logger.warning(
                        f"⚠️  [USDJPY_CHECK] ATTENTION: pip_size calculé = {pip_size}"
                    )
                    logger.warning(f"    USDJPY devrait avoir 1 pip = 0.01")
                    logger.warning(f"    Vérifiez point={point}, digits={digits}")
                else:
                    logger.info(f"✅ [USDJPY_CHECK] OK: 1 pip = 0.01")

            # Vérification générale
            if abs(pip_size - expected_pip) > 0.000001:
                logger.warning(f"⚠️  [PIP_CALC_WARNING][{sym}] Différence détectée!")
                logger.warning(f"    Calculé: {pip_size} vs Attendu: {expected_pip}")
                logger.warning(f"    Delta: {abs(pip_size - expected_pip)}")
    ...
    def _snapshot_positions():
        try:
            ...
            # ✅ DEBUG TEMP : comprendre pourquoi le monitor ne "voit" rien
            if logger:
                logger.info(f"[BASKET_DEBUG] MT5 positions total: {len(allp)} | BOT_MAGIC={BOT_MAGIC}")

                shown = 0
                for p in allp:
                    if shown >= 10:
                        break
                    sym = str(_v(p, "symbol", "") or "")
                    mg  = _v(p, "magic", None)
                    cm  = str(_v(p, "comment", "") or "")
                    is_bot = False
                    try:
                        is_bot = (int(mg) == int(BOT_MAGIC)) and bool(BASKET_TAG_RE.search(cm))
                    except Exception:
                        pass

                    logger.info(f"[BASKET_DEBUG] pos sym={sym} ticket={_v(p,'ticket')} magic={mg} comment='{cm}' -> is_bot={is_bot}")
                    shown += 1

            return [p for p in allp if _is_bot_pos(p)]
        except Exception as e:
            if logger:
                logger.error(f"[BASKET_DEBUG] get_positions error: {e}")
            return []
    ...
    # FIX: On force le démarrage si enable_profit_close est True, même si window est 0 par erreur
    if enable_profit_close and rt_poll_interval_ms > 0:
        if rt_fast_window_ms <= 0:
            rt_fast_window_ms = 86400000  # 24h par défaut si 0
        deadline = time.monotonic() + (rt_fast_window_ms / 1000.0)
        loop_count = 0
        last_log_ts = {}  # Timestamp du dernier log par basket (éviter spam)

        # ✅ CORRECTION AJOUTÉE : Log de configuration CRITIQUE
        if logger:
            logger.info("=" * 80)
            logger.info("🔧 [BASKET_MONITOR_CONFIG] Configuration chargée:")
            logger.info(f"   • enable_profit_close: {enable_profit_close}")
            logger.info(
                f"   • target_profit_pips (fallback): {target_profit_pips if target_profit_pips is not None else 'N/A'}"
            )
            logger.info(f"   • max_loss_pips (fallback): {max_loss_pips}")
            logger.info(
                f"   • closure['target_profit_pips']: {closure.get('target_profit_pips', 'N/A')}"
            )
            logger.info("=" * 80)
        ...
        while True:
            ...
            for basket_id, pos in baskets.items():
                ...
                # =========================================================
                # 1) STATS D'ABORD (sym requis pour charger config asset)
                # =========================================================
                ...
                # =========================================================
                # 2) CHARGER RÈGLES PAR ACTIF (ordre de priorité CORRIGÉ)
                #    🔧 FIX 23 JAN 2026: Le config_manager est maintenant
                #    PRIORITAIRE pour charger les valeurs SPÉCIFIQUES au
                #    symbole du BASKET (pas du worker qui appelle).
                #    Le paramètre devient un FALLBACK de sécurité.
                #
                #    NOUVELLE priorité:
                #    1. config_manager pour LE SYMBOLE DU BASKET ← PRIORITÉ!
                #    2. paramètre explicite (fallback global)
                #    3. closure dict (fallback legacy)
                #    4. hard default (filet de sécurité)
                # =========================================================
                asset_target_profit = None
                asset_max_loss = None
                source_profit = "unset"
                source_loss = "unset"

                # 2.1 PRIORITÉ 1: config_manager pour LE SYMBOLE DU BASKET
                # C'est LA source de vérité pour les valeurs par asset
                cm_local = (
                    config_manager_instance
                    if config_manager_instance
                    else getattr(self, "config_manager", None)
                )
                if cm_local and hasattr(cm_local, "get_closure_rules_config"):
                    try:
                        specific = (
                            cm_local.get_closure_rules_config(sym, "scalping") or {}
                        )
                        # Charger target_profit_pips pour CE symbole
                        if "target_profit_pips" in specific:
                            asset_target_profit = float(specific["target_profit_pips"])
                            source_profit = "config_manager"
                        # Charger max_loss_pips pour CE symbole
                        if "max_loss_pips" in specific:
                            asset_max_loss = float(specific["max_loss_pips"])
                            source_loss = "config_manager"
                    except Exception as e:
                        if logger:
                            logger.error(
                                f"❌ [CONFIG_ERROR] {sym} get_closure_rules_config: {e}"
                            )

                # 2.2 PRIORITÉ 2: paramètre explicite (FALLBACK global)
                # Utilisé seulement si config_manager n'a pas trouvé de valeur
                if asset_target_profit is None and target_profit_pips is not None:
                    asset_target_profit = float(target_profit_pips)
                    source_profit = "param_fallback"

                if asset_max_loss is None and max_loss_pips is not None:
                    asset_max_loss = float(max_loss_pips)
                    source_loss = "param_fallback"

                # 2.3 PRIORITÉ 3: closure dict (legacy fallback)
                if (
                    asset_target_profit is None
                    and closure is not None
                    and "target_profit_pips" in closure
                ):
                    asset_target_profit = float(closure.get("target_profit_pips", 15.0))
                    source_profit = "closure_fallback"

                if (
                    asset_max_loss is None
                    and closure is not None
                    and "max_loss_pips" in closure
                ):
                    asset_max_loss = float(closure.get("max_loss_pips", 15.0))
                    source_loss = "closure_fallback"

                # 2.4 PRIORITÉ 4: dernier filet de sécurité
                if asset_target_profit is None:
                    asset_target_profit = 15.0
                    source_profit = "hard_default"

                if asset_max_loss is None:
                    asset_max_loss = 15.0
                    source_loss = "hard_default"

                # =========================================================
                # 3) LOG: afficher la valeur EFFECTIVEMENT utilisée + source
                # =========================================================
                if logger:
                    logger.info(
                        f"🎛️ [CLOSURE_RULES_USED] {basket_id} {sym} | "
                        f"profit={asset_target_profit}p ({source_profit}) | "
                        f"loss={asset_max_loss}p ({source_loss}) | "
                        f"pnl={pnl_pips:+.2f}p"
                    )

                # ✅ CORRECTION : LOG CRITIQUE DE COMPARAISON
                if logger:
                    logger.info("=" * 60)
                    logger.info(f"🎯 [PROFIT_CHECK_CRITICAL] {basket_id}")
                    logger.info(f"   Symbol: {sym} | Direction: {direction}")
                    logger.info(f"   Entry: {avg_entry:.5f} | Current: {avg_price:.5f}")
                    logger.info(f"   Différence prix: {(avg_price - avg_entry):.5f}")
                    logger.info(f"   Taille pip: {pip_size}")
                    logger.info(f"   PnL calculé: {pnl_pips:.2f} pips")
                    logger.info(f"   Target config: {asset_target_profit:.2f} pips")
                    logger.info(
                        f"   Condition: {pnl_pips:.2f} >= {asset_target_profit:.2f} = {pnl_pips >= asset_target_profit}"
                    )
                    if pnl_pips >= asset_target_profit:
                        logger.info("   ⚠️  ⚠️  ⚠️  FERMETURE DÉCLENCHÉE! ⚠️  ⚠️  ⚠️")
                    logger.info("=" * 60)
                ...
                if now - last_log >= 5.0:
                    ...
                    # AJOUTEZ CE DEBUG DÉTAILLÉ
                    logger.info(f"🎯 [PROFIT_CHECK_DETAILED] {basket_id}")
                    logger.info(f"   - Symbol: {sym}")
                    logger.info(f"   - Direction: {direction}")
                    logger.info(f"   - Entry price: {avg_entry:.5f}")
                    logger.info(f"   - Current price: {avg_price:.5f}")
                    logger.info(f"   - Price difference: {avg_price - avg_entry:.5f}")
                    logger.info(f"   - Pip size: {pip_size}")
                    logger.info(f"   - PnL pips: {pnl_pips:.2f}")
                    logger.info(f"   - Target pips: {asset_target_profit:.2f}")
                    logger.info(f"   - Should close: {pnl_pips >= asset_target_profit}")
                    logger.info(
                        f"   - Condition: {pnl_pips:.2f} >= {asset_target_profit:.2f} = {pnl_pips >= asset_target_profit}"
                    )
                ...
                if pnl_pips >= asset_target_profit:
                    ...
                    # ✅ CORRECTION : AJOUTER LA VALEUR UTILISÉE
                    logger.info(
                        f"   🔧 target_profit_pips UTILISÉ: {asset_target_profit:.2f} pips"
                    )
                    ...
                    if _close_basket(basket_id, pos):
                        ...
                        logger.error(
                            f"   📊 PnL: {pnl_pips:+.2f} pips | Target: {asset_target_profit:.2f} pips"
                        )

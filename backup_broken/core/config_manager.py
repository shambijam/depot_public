...
class ConfigManager:
    ...
    def load_asset_config(self, asset: str) -> Dict[str, Any]:
            """
            Charge et met en cache la config d'un actif (NAS100.json, etc.).
            Version CORRIGÉE : Chemins absolus + Gestion Majuscules/Minuscules.
            """
            if not hasattr(self, "_asset_config_cache"):
                self._asset_config_cache: Dict[str, Dict[str, Any]] = {}

            # Normalisation pour le cache (on stocke tout en majuscule par convention interne)
            asset_key = str(asset).upper()
            if asset_key in self._asset_config_cache:
                return self._asset_config_cache[asset_key]

            # 1. Construction du chemin ABSOLU basé sur l'emplacement de ce fichier (core/config_manager.py)
            # On remonte d'un cran (core/..) pour trouver la racine, puis on va dans config/assets_config
            base_dir = Path(__file__).resolve().parent.parent
            config_dir_rel = self.get("paths.asset_configs", "config/assets_config/")
            
            # Nettoyage du chemin relatif si nécessaire (retirer le slash initial s'il y est)
            if str(config_dir_rel).startswith("/") or str(config_dir_rel).startswith("\\"):
                config_dir_rel = str(config_dir_rel)[1:]
                
            assets_dir = base_dir / config_dir_rel

            if not assets_dir.exists():
                self.logger.error(f"Dossier de configs introuvable: {assets_dir}")
                return {}

            # 2. Stratégie de recherche multi-casse (NAS100.json, nas100.json, etc.)
            candidates = [
                f"{asset}.json",          # Nom exact demandé
                f"{asset.upper()}.json",  # NAS100.json (Votre cas)
                f"{asset.lower()}.json",  # nas100.json
            ]

            target_file = None
            for candidate in candidates:
                temp_path = assets_dir / candidate
            ...
            if not target_file:
                self.logger.warning(
                    f"Config introuvable pour {asset}. Cherché dans {assets_dir} "
                    f"avec les noms : {candidates}"
                )
                # On met un cache vide pour éviter de spammer le disque
                self._asset_config_cache[asset_key] = {}
                return {}
            ...
            try:
                ...
                self._asset_config_cache[asset_key] = cfg
                self.logger.info(f"✅ Config chargée pour {asset} depuis {target_file.name}")
                return cfg

            except Exception as e:
                self.logger.error(f"Erreur lecture JSON pour {asset}: {e}", exc_info=True)
                return {}
    ...
    def get_closure_rules_config(self, asset_symbol: str, strategy_name: str = "scalping") -> Dict[str, Any]:
        """
        Raccourci pour obtenir la config closure_rules fusionnée pour un actif.

        Returns:
            Dict avec: enabled, target_profit_pips, max_loss_pips, etc.
        """
        return self.config_merger.get_closure_rules_config(asset_symbol, strategy_name)
    
    def debug_merge_process(self, asset_symbol: str, strategy_name: str = "scalping") -> None:
        """
        Proxy: permet d'appeler debug_merge_process depuis config_manager.
        Utile car la méthode vit dans ConfigMerger (config_merge.py).
        """
        if not hasattr(self, "config_merger") or self.config_merger is None:
            self.logger.warning("⚠️ ConfigManager.debug_merge_process: config_merger non initialisé")
            return

        if not hasattr(self.config_merger, "debug_merge_process"):
            self.logger.warning("⚠️ ConfigManager.debug_merge_process: ConfigMerger n'a pas debug_merge_process")
            return

        return self.config_merger.debug_merge_process(asset_symbol, strategy_name)

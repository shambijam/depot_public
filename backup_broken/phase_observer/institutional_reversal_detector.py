...
class InstitutionalReversalDetector:
    ...
    def _default_config(self) -> Dict:
        ...
        return {
            ...
            "signals": {
                # Poids ajustés (6 couches = 100%)
                "changepoint_weight": 0.30,       # +5% (importance élevée)
                "divergence_weight": 0.25,        # +5% (M1+M5)
                "fatigue_weight": 0.15,           # Inchangé
                "accumulation_weight": 0.15,      # Inchangé (Wyckoff)
                "microstructure_m1_weight": 0.08, # Réduit de 10% (sans ticks)
                "pattern_weight": 0.07            # Inchangé (ML)
            },
            "thresholds": {

...
class DynamicStrategy(BaseStrategy):
    """
    Moteur de stratégie pour le modèle 'Dynamic'.
    Exécute les règles de sa configuration pour s'adapter à diverses conditions de marché.
    """
    ...
    def __init__(self, config_manager_instance: Any, strategy_config: Dict[str, Any]):
        ...
        self.logger.info("Moteur de stratégie Dynamic initialisé.")
        ...
        for position in current_positions:
            ...
            for rule in exit_rules:
                if self._check_rule_conditions(asset_signals, rule.get("conditions", {})):
                    self.logger.info(f"RÈGLE DE SORTIE DYNAMIC: '{rule.get('name')}' pour la position #{position['ticket']} sur {asset}.")
    ...
    def update_strategy_parameters(self, new_params: Dict[str, Any]) -> None:
        ...
        self.logger.info(f"Mise à jour des paramètres de la stratégie Dynamic : {new_params}")

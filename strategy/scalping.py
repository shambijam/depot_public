...
class ScalpingStrategy(BaseStrategy):
    ...
    def evaluate_entry(self, context: Dict[str, Any], signals: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        """
        self.logger.debug("ScalpingStrategy: recherche du meilleur candidat pour l'entrée...")
        ...
        if best_asset is None:
            self.logger.info("Aucun signal d'actif jugé suffisant pour une entrée de scalping.")
            return None

        self.logger.info(f"MEILLEUR CANDIDAT TROUVÉ: {best_asset} avec un score de {highest_score:.2f}.")
        ...
        if "bullish" in phase or "up" in phase or "expansion_up" in phase:
            ...
            self.logger.warning(f"Phase '{phase}' non conclusive pour {best_asset}. Trade annulé.")
            return None

...
# ================================================================
# 🚫 FONCTIONS DE VETO RANGE/ACCUMULATION - 26 Décembre 2025
# ================================================================
def veto_range_usdjpy(
    ...
    Args:
        df_m1: DataFrame M1 avec colonnes 'high' et 'low'
        threshold_pips: Seuil en décimal (0.0003 = ~3 pips USDJPY)
        lookback_bars: Nombre de bougies à analyser (défaut: 5)
    ...
    Exemple USDJPY:
        - 5 dernières bougies avec range moyen 0.00025 (2.5 pips) → VETO
        - 5 dernières bougies avec range moyen 0.00045 (4.5 pips) → OK
    """
    ...
    # Range moyen = (high - low) moyen sur les N dernières bougies
    avg_range = (recent['high'] - recent['low']).mean()

    # Si range moyen < seuil → marché en consolidation serrée
...
def veto_accumulation_usdjpy(
    ...
    Args:
        vp_data: Dict du volume profile avec 'va_low', 'va_high', 'price_min', 'price_max'
        threshold: Ratio VA/Range déclenchant le veto (défaut: 0.6 = 60%)
    ...
    # Ratio VA / Range total
    va_ratio = va_width / price_range

    # Si VA occupe > threshold % du range → marché équilibré (accumulation)

...
class LiquidityStrategy(BaseStrategy):
    ...
    def evaluate_entry(self, context: Dict[str, Any], signals: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        """
        best_asset = None
        # On peut garder un seuil de sécurité, mais il doit être raisonnable.
        ...
        self.logger.info(f"MEILLEUR CANDIDAT LIQUIDITY: {best_asset} (Confiance: {highest_confidence:.2f}).")
        ...
        # On simule une "règle" pour que la fonction _build_decision_package fonctionne
        mock_rule = {'name': f"Liquidity entry for {best_asset}", "order_type": "MARKET"}
